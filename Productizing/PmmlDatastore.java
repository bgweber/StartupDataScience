...
public class PmmlDataStore {
  ...
  public static void main(String[] args) {
    ...
    .apply("To Entities", ParDo.of(new DoFn<TableRow, Entity>() {          
	  ...
	  public void processElement(ProcessContext c) throws Exception {
        ...
        Key.PathElement.Builder path = Key.PathElement.newBuilder().setKind("Profile").setName(keyName);
        keyBuilder.addPath(path);	        	
        Key key = keyBuilder.build();

        // set the experiment group
        String expGroup = Double.parseDouble(row.get("predicted_weight").toString()) >= 8 ? "Control" : "Treatment"; 
        Value value = Value.newBuilder().setStringValue(expGroup).build();
